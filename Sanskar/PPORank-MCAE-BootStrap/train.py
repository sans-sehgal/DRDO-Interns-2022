...
def get_name(datadir):
    ...
    for i in lst:
        if('ohsumed' in i.lower()):
            ...
            ds='MSLR'
        elif('complete_msmarco_' in i.lower()):
            ds='MSMarco'
    if(len(ds)==0):
...
def train(datadir,n_features,n_iterations,max_ep_len,save_model_freq,update_timestep,K_epochs,eps_clip,gamma,lr_actor,lr_critic,hidden_nodes,random_seed):
    ...
    results={'train':[],'test':[]}
    datadir_new = datadir
    while minibatch_number<=125:
        print("===== currently training minibatch ", minibatch_number, " =====")
        ...
        i_episode = 0
        datadir_new = datadir
        datadir_new+=str(minibatch_number)
        #Load the data object
        data = Dataset(datadir_new)
